...
ptope_t* random_ptope(int dim,float frac)
{
  ptope_t* o = ptope_alloc_internal(pr,dim,0);
  bound_t* b;
  int i,j,x,y;
  num_t n;
...
/* build polyhedron & parallelotopes at the same time, from same constraints */
void random_poly_ptope(int dim,float frac, ptope_t**o, ap_abstract0_t** p)
{
  bound_t* b;
  int i,j,x,y;
  num_t n;
  *o = ptope_alloc_internal(pr,dim,0);
  (*o)->m = hmat_alloc_top(pr,dim);
...
ptope_t* ptope_of_poly(ap_abstract0_t* p)
{
  ...
  FLAG(mp);
  o = ptope_of_generator_array(mo,d.intdim,d.realdim,&ar);
  FLAG(mo);
  /*ap_generator0_array_fprint(stderr,&ar,NULL);*/
  ap_generator0_array_clear(&ar);
  return o;
...
/* may not be exact (e.g., long double parallelotopes and mpq polyhedra) */
ap_abstract0_t* poly_of_ptope(ptope_t* o)
{
  ap_lincons0_array_t ar;
  ap_dimension_t d = ptope_dimension(mo,o);
  ap_abstract0_t* p;
  ar = ptope_to_lincons_array(mo,o);
  FLAG(mo);
...
void print_ptope(const char* msg, ptope_t* o)
{
  fprintf(stderr,"%s = ",msg);
  ptope_fprint(stderr,mo,o,NULL);
  ptope_fdump(stderr,mo,o);
  fprintf(stderr,"\n");
}
...
bool ptope_is_nleq(ap_manager_t* man, ptope_t* a1, ptope_t* a2)
{
  return !ptope_is_leq(man,a1,a2) && man->result.flag_exact;
}


bool ptope_is_neq(ap_manager_t* man, ptope_t* a1, ptope_t* a2)
{
  return !ptope_is_eq(man,a1,a2) && man->result.flag_exact;
}
...
void test_misc(void)
{
  size_t D = 5;
  int i;
  ptope_t* bot = ptope_bottom(mo,0,D);
  ptope_t* top = ptope_top(mo,0,D);
  ap_abstract0_t* bot2 = poly_of_ptope(bot);
  ap_abstract0_t* top2 = poly_of_ptope(top);
  ap_dimension_t d1 = ptope_dimension(mo,bot);
  ap_dimension_t d2 = ptope_dimension(mo,top);
  printf("\nperforming various tests\n");
  check(bot); check(top);
  if (d1.intdim || d1.realdim!=D) printf("ptope_dimension failed #1\n");
  if (d2.intdim || d2.realdim!=D) printf("ptope_dimension failed #2\n");
  if (!ptope_is_bottom(mo,bot))  printf("ptope_is_bottom failed #3\n");
  if (ptope_is_bottom(mo,top))   printf("ptope_is_bottom failed #4\n");
  if (ptope_is_top(mo,bot))      printf("ptope_is_top failed #5\n");
  if (!ptope_is_top(mo,top))     printf("ptope_is_top failed #6\n");
  if (!ptope_is_leq(mo,bot,top)) printf("ptope_is_leq failed #7\n");
  if (ptope_is_leq(mo,top,bot))  printf("ptope_is_leq failed #8\n");
  if (!ptope_is_eq(mo,bot,bot))  printf("ptope_is_eq failed #9\n");
  if (!ptope_is_eq(mo,top,top))  printf("ptope_is_eq failed #10\n");
  if (ptope_is_eq(mo,bot,top))   printf("ptope_is_eq failed #11\n");
  if (ptope_is_dimension_unconstrained(mo,bot,0))
    printf("ptope_is_dimension_unconstrained #12\n");
  if (!ptope_is_dimension_unconstrained(mo,top,0))
    printf("ptope_is_dimension_unconstrained #13\n");
  ...
  for (i=0;i<N;i++) {
    ptope_t* o = random_ptope(D,.1);
    ptope_t* c = ptope_copy(mo,o);
    ptope_t* l = ptope_closure(mo,false,o);
    ap_dimension_t d = ptope_dimension(mo,o);
    if (d.intdim || d.realdim!=D) printf("ptope_dimension failed #14\n");
    if (!ptope_is_leq(mo,bot,o))  printf("ptope_is_leq failed #15\n");
    if (!ptope_is_leq(mo,o,top))  printf("ptope_is_leq failed #16\n");
    if (!ptope_is_eq(mo,o,c))     printf("ptope_is_eq failed #17\n");
    if (ptope_is_neq(mo,o,l))     printf("ptope_is_eq failed #18\n");
    ptope_size(mo,o);
    ptope_close(pr,o);
    // not implemented
    //ptope_minimize(mo,o);
    //ptope_canonicalize(mo,o);
    //ptope_approximate(mo,o,0);
    //ptope_is_minimal(mo,o);
    //ptope_is_canonical(mo,o);
    ptope_free(mo,o); ptope_free(mo,c); ptope_free(mo,l);
  }
  ptope_free(mo,bot); ptope_free(mo,top);
...
void test_incremental_closure(void)
{
  ...
  LOOP {
    size_t dim = 14;
    ptope_t* o = random_ptope(dim,.005);
...
void test_polyhedra_conversion(void)
{
  ...
  LOOP {
    ...
    if (ptope_is_nleq(mo,o,o2)) {
      ERROR("ptope not included in");
      print_ptope("o",o);
      print_poly("p",p);
      print_ptope("o2",o2);
    }
    ...
    if (ptope_is_eq(mo,o,o2) && ap_abstract0_is_eq(mp,p,p2)) RESULT('*');
    if (flag>=best && (ptope_is_neq(mo,o,o2) || !ap_abstract0_is_eq(mp,p,p2))) ERROR("best flag");
    ptope_free(mo,o); ptope_free(mo,o2); ap_abstract0_free(mp,p); ap_abstract0_free(mp,p2);
  } ENDLOOP;
  {
    ptope_t *o;
    ap_abstract0_t *p;
...
void test_polyhedra_conversion2(void)
{
  ...
  LOOP {
    ...
    o2 = ptope_of_poly(p2);
    RESULT(check(o)); check(o2);
    if (ptope_is_nleq(mo,o,o2) || !ap_abstract0_is_leq(mp,p,p2)) {
    ...
    if (ptope_is_eq(mo,o,o2)) RESULT('*');
    if (flag>=best && ptope_is_neq(mo,o,o2)) ERROR("best flag");
    ptope_free(mo,o); ptope_free(mo,o2); ap_abstract0_free(mp,p); ap_abstract0_free(mp,p2);
  } ENDLOOP;
  {
    ap_abstract0_t *p;
    ...
    o = ptope_of_poly(p);
    if (flag>=best && !ptope_is_bottom(mo,o)) printf("ptope_is_bottom failed\n");
    ptope_free(mo,o); ap_abstract0_free(mp,p);
  }
  {
    ap_abstract0_t *p;
    ...
    o = ptope_of_poly(p);
    if (flag>=best && !ptope_is_top(mo,o)) printf("ptope_is_top failed\n");
    ptope_free(mo,o); ap_abstract0_free(mp,p);
  }
}
...
void test_lincons_conversion(exprmode mode)
{
  printf("\nconversion from %slincons %s\n",exprname[mode],
	 num_incomplete?"":(mode<=expr_ptope)?"(* expected)":"(*,x,. expected)");
  LOOP {
    int dim = 6, nb = 10, i;
    ap_abstract0_t *p, *p2;
    ptope_t *o, *oo;
    ...
    o  = ptope_top(mo,0,dim);
    o  = ptope_meet_lincons_array(mo,true,o,&t); FLAG(mo);
    p2 = poly_of_ptope(o);
    oo = ptope_of_poly(p);
    RESULT(check(o));
    if (!ap_abstract0_is_leq(mp,p,p2)) {
    ...
    else if (ptope_is_eq(mo,o,oo)) RESULT('x');
    if (flag>=best && ptope_is_neq(mo,o,oo)) ERROR("best flag");
    if (flag==exact && !ap_abstract0_is_eq(mp,p,p2)) ERROR("exact flag");
    ptope_free(mo,o); ptope_free(mo,oo);
...
void test_generator_conversion(void)
{
  ...
  LOOP {
    ...
    o  = random_ptope(dim,.1);
    t  = ptope_to_generator_array(mo,o); FLAG(mo);
    o2 = ptope_of_generator_array(mo,0,dim,&t); FLAG(mo);
    RESULT(check(o2));
    if (ptope_is_nleq(mo,o,o2)) {
      ERROR("not included in");
      print_ptope("o",o);
    ...
    if (ptope_is_eq(mo,o,o2)) RESULT('*');
    if (flag==exact && ptope_is_neq(mo,o,o2)) ERROR("exact flag");
    ptope_free(mo,o); ptope_free(mo,o2); ap_generator0_array_clear(&t);
  } ENDLOOP;
}
...
void test_box_conversion(void)
{
  ...
  LOOP {
    ...
    ap_interval_t **b,**b2;
    o = random_ptope(dim,.1);
    b = ptope_to_box(mo,o); FLAG(mo);
    o2 = ptope_of_box(mo,0,dim,b); FLAG(mo);
    b2 = ptope_to_box(mo,o2); FLAG(mo);
    o3 = ptope_of_box(mo,0,dim,b2); FLAG(mo);
    RESULT(check(o)); check(o2);
    if (ptope_is_nleq(mo,o,o2) || ptope_is_nleq(mo,o2,o3)) {
      ERROR("not included in");
      print_ptope("o",o);
      print_ptope("o2",o2);
      print_ptope("o3",o3);
    }
    if (ptope_is_eq(mo,o2,o3)) RESULT('*');
    if (flag==exact && ptope_is_neq(mo,o2,o3)) ERROR("exact flag");
    ap_interval_array_free(b,dim);
    ap_interval_array_free(b2,dim);
    ptope_free(mo,o); ptope_free(mo,o2); ptope_free(mo,o3);
  } ENDLOOP;
}
...
void test_serialize(void)
{
  ...
  LOOP {
    ...
    ap_membuf_t b;
    o  = random_ptope(10,.1);
    b  = ptope_serialize_raw(mo,o); FLAG(mo);
    o2 = ptope_deserialize_raw(mo,b.ptr,&sz); FLAG(mo);
    RESULT(check(o)); check(o2);
    if (ptope_is_nleq(mo,o,o2)) {
      ERROR("not included in");
      print_ptope("o",o);
    ...
    ptope_free(mo,o); ptope_free(mo,o2); free(b.ptr);
  } ENDLOOP;
}
...
void test_bound_dim(void)
{
  ...
  LOOP {
    ...
    ap_abstract0_t *p;
    ap_interval_t *io, *ip;
    random_poly_ptope(dim, .2, &o, &p);
    io = ptope_bound_dimension(mo,o,v); FLAG(mo);
    ip = ap_abstract0_bound_dimension(mp,p,v); FLAG(mp);
    RESULT(check(o));
    if (!ptope_sat_interval(mo,o,v,io)) ERROR("not sat ptope");
    FLAG(mo);
...
{
  printf("\nbound %slinexpr %s\n",exprname[mode],
	 num_incomplete?"":(mode<=expr_ptope)?"(* expected)":"(*,. expected)");
  LOOP {
    int dim = 6;
    ptope_t *o;
    ap_abstract0_t *p;
    ...
    io = ptope_bound_linexpr(mo,o,e); FLAG(mo);
...
void test_bound_texpr(void)
{
  ...
  LOOP {
    ...
    io = ptope_bound_texpr(mo,o,e); FLAG(mo);
...
void test_meet(void)
{
  ...
  LOOP {
    ...
    random_poly_ptope(dim, .2, &o1, &p1);
    random_poly_ptope(dim, .2, &o2, &p2);
    o  = ptope_meet(mo,false,o1,o2); FLAG(mo);
    p  = ap_abstract0_meet(mp,false,p1,p2); FLAG(mp);
    pp = poly_of_ptope(o);
    RESULT(check(o)); check(o1); check(o2);
    if (ptope_is_nleq(mo,o,o1) || ptope_is_nleq(mo,o,o2)) {
      ERROR("not lower bound");
      print_ptope("o1",o1); print_ptope("o2",o2); print_ptope("o",o);
      print_poly("p",p);
    }
    ...
    ptope_free(mo,o); ptope_free(mo,o1); ptope_free(mo,o2);
  ...
  LOOP {
    int dim = 8;
    ptope_t *o1, *o2, *o, *oo;
    o1 = random_ptope(dim,.2);
    o2 = ptope_top(mo,0,dim);
    o  = ptope_meet(mo,false,o1,o2);
    oo = ptope_meet(mo,false,o,o1);
    check(o1); check(o2); check(o);
    if (ptope_is_neq(mo,o,o1)) {
      ERROR("not eq #1");
      print_ptope("o1",o1); print_ptope("o",o);
    }
    else if (ptope_is_neq(mo,o,oo)) {
      ERROR("not eq #2");
      print_ptope("o1",o1); print_ptope("o",o); print_ptope("oo",oo);
    }
    else RESULT('*');
    ptope_free(mo,o); ptope_free(mo,o1); ptope_free(mo,o2); ptope_free(mo,oo);
  } ENDLOOP;
  printf("\nmeet bot %s\n","(* expected)");
  LOOP {
    int dim = 8;
    ptope_t *o1, *o2, *o;
    o1 = random_ptope(dim,.2);
    o2 = ptope_bottom(mo,0,dim);
    o  = ptope_meet(mo,false,o1,o2);
    check(o1); check(o2); check(o);
    if (!ptope_is_bottom(mo,o)) {
      ERROR("not bottom");
      print_ptope("o1",o1); print_ptope("o",o);
    }
    else RESULT('*');
...
void test_meet_array(void)
{
  ...
  LOOP {
    int i, dim = 6;
    ptope_t* o[NB_MEET], *oo;
    ap_abstract0_t* p[NB_MEET], *pp, *ppp;
    for (i=0;i<NB_MEET;i++)
      random_poly_ptope(dim, .2, &o[i], &p[i]);
    oo = ptope_meet_array(mo,o,NB_MEET); FLAG(mo);
    pp = poly_of_ptope(oo);
    ppp = ap_abstract0_meet_array(mp,p,NB_MEET);
    FLAG(mp);
    RESULT(check(oo));
    for (i=0;i<NB_MEET;i++)
      if (ptope_is_nleq(mo,oo,o[i])) ERROR("not lower bound");
    ...
    for (i=0;i<NB_MEET;i++) { ptope_free(mo,o[i]); ap_abstract0_free(mp,p[i]); }
    ptope_free(mo,oo); ap_abstract0_free(mp,pp); ap_abstract0_free(mp,ppp);
  } ENDLOOP;
}
...
void test_add_lincons(exprmode mode)
{
  ...
  LOOP {
    ...
    random_poly_ptope(dim, .2, &o, &p);
    if (lrand48()%10>=8) ptope_close(pr,o);
    for (i=0;i<nb;i++) {
    ...
    o1 = ptope_meet_lincons_array(mo,false,o,&ar); FLAG(mo);
    ...
    if (!ap_abstract0_is_leq(mp,p1,p2)) {
      ...
      print_ptope("o",o); print_ptope("o1",o1); print_ptope("o2",o2);
    }
    if (ap_abstract0_is_eq(mp,p1,p2)) RESULT('*');
    else if (ptope_is_eq(mo,o1,o2)) RESULT('x');
    if (flag>=best && !ap_abstract0_is_eq(mp,p1,p2)) ERROR("best flag");
    if (flag==exact && ptope_is_neq(mo,o1,o2)) ERROR("exact flag");
    ptope_free(mo,o); ptope_free(mo,o1); ptope_free(mo,o2);
...
void test_sat_lincons(exprmode mode)
{
  ...
  LOOP {
    ...
    o1 = ptope_meet_lincons_array(mo,false,o,&ar); FLAG(mo);
    RESULT(check(o1));
    ro  = ptope_sat_lincons(mo,o,&l); FLAG(mo);
    ro1 = ptope_sat_lincons(mo,o1,&l); FLAG(mo);
    ...
    ptope_free(mo,o); ptope_free(mo,o1); ap_abstract0_free(mp,p);
...
void test_join(void)
{
  ...
  LOOP {
    ...
    ptope_t *o1, *o2, *o3, *o;
    ap_abstract0_t *p1, *p2, *p, *pp;
    random_poly_ptope(dim, .1, &o1, &p1);
    random_poly_ptope(dim, .1, &o2, &p2);
    o  = ptope_join(mo,false,o1,o2); FLAG(mo);
    p  = ap_abstract0_join(mp,false,p1,p2); FLAG(mp);
    pp = poly_of_ptope(o);
    o3 = ptope_of_poly(p);
    RESULT(check(o)); check(o1); check(o2);
    if (ptope_is_nleq(mo,o1,o) || ptope_is_nleq(mo,o2,o)) {
    ...
    if (ap_abstract0_is_eq(mp,p,pp)) RESULT('*');
    else if (ptope_is_eq(mo,o,o3)) RESULT('x');
    if (flag==exact && !ap_abstract0_is_eq(mp,p,pp)) ERROR("exact flag");
    if (flag>=best && ptope_is_neq(mo,o,o3)) ERROR("best flag");
    ptope_free(mo,o); ptope_free(mo,o1); ptope_free(mo,o2); ptope_free(mo,o3);
    ap_abstract0_free(mp,p); ap_abstract0_free(mp,pp); ap_abstract0_free(mp,p1); ap_abstract0_free(mp,p2);
  } ENDLOOP;
  printf("\njoin bot %s\n",num_incomplete?"":"(* expected)");
  LOOP {
    ...
    o1 = random_ptope(dim,.2);
    o2 = ptope_bottom(mo,0,dim);
    o  = ptope_join(mo,false,o1,o2);
    oo = ptope_join(mo,false,o,o1);
...
void test_join_array(void)
{
  ...
  LOOP {
    int i, dim = 6;
    ptope_t* o[NB_JOIN], *oo, *ooo;
    ap_abstract0_t* p[NB_JOIN], *pp, *ppp, *ps;
    for (i=0;i<NB_JOIN;i++)
      random_poly_ptope(dim, .1, &o[i], &p[i]);
    oo = ptope_join_array(mo,o,NB_JOIN); FLAG(mo);
    pp = poly_of_ptope(oo);
    ...
    ooo = ptope_of_poly(ps);
    RESULT(check(oo));
    for (i=0;i<NB_JOIN;i++)
      if (ptope_is_nleq(mo,o[i],oo)) ERROR("not upper bound");
    if (!ap_abstract0_is_leq(mp,ppp,pp)) {
      ERROR("not poly approx");
      for (i=0;i<NB_JOIN;i++)
	{ char n[3] = { 'o', '0'+i, 0 }; print_ptope(n,o[i]); }
      for (i=0;i<NB_JOIN;i++)
    ...
    if (ap_abstract0_is_eq(mp,ppp,pp)) RESULT('*');
    else if (ptope_is_eq(mo,oo,ooo)) RESULT('x');
    if (flag==exact && !ap_abstract0_is_eq(mp,ppp,pp)) ERROR("exact flag");
    if (flag>=best && ptope_is_neq(mo,oo,ooo)) ERROR("best flag");
    ...
    for (i=0;i<NB_JOIN;i++) { ptope_free(mo,o[i]); ap_abstract0_free(mp,p[i]); }
    ptope_free(mo,oo); ptope_free(mo,ooo);
...
void test_add_ray(void)
{
  ...
  LOOP {
    ...
    o1 = ptope_add_ray_array(mo,false,o,&ar); FLAG(mo);
    ...
    if (flag>=best && ptope_is_neq(mo,o1,o2)) ERROR("best flag");
    ptope_free(mo,o); ptope_free(mo,o1); ptope_free(mo,o2);
    ap_abstract0_free(mp,p); ap_abstract0_free(mp,p1); ap_abstract0_free(mp,p2);
    ap_generator0_array_clear(&ar);
   } ENDLOOP;
...
void test_dimadd(void)
{
  ...
  LOOP {
    ...
    o1 = random_ptope(dim,.1);
    if (lrand48()%10>=8) ptope_close(pr,o1);
    for (i=0;i<a->realdim;i++) {
      a->dim[i] = lrand48()%3;
      if (i) a->dim[i] += a->dim[i-1];
      r->dim[i] = a->dim[i]+i;
    }
    o2 = ptope_add_dimensions(mo,false,o1,a,true);
    o3 = ptope_remove_dimensions(mo,false,o2,r);
    RESULT(check(o1)); check(o2); check(o3);
    if (ptope_is_neq(mo,o1,o3)) {
      ERROR("not eq");
      ap_dimchange_fprint(stderr,a); ap_dimchange_fprint(stderr,r);
      print_ptope("o1",o1); print_ptope("o2",o2);print_ptope("o3",o3);
    }
    ptope_free(mo,o1); ptope_free(mo,o2); ptope_free(mo,o3);
...
void test_dimrem(void)
{
  ...
  LOOP {
    ...
    o2 = ptope_remove_dimensions(mo,false,o1,r);
    o3 = ptope_add_dimensions(mo,false,o2,a,false);
    o4 = ptope_forget_array(mo,false,o1,r->dim,r->realdim,false);
    RESULT(check(o1)); check(o2); check(o3); check(o4);
    if (ptope_is_neq(mo,o3,o4)) {
      ERROR("not eq");
      ap_dimchange_fprint(stderr,r); ap_dimchange_fprint(stderr,a);
      print_ptope("o1",o1); print_ptope("o2",o2);
      print_ptope("o3",o3); print_ptope("o4",o4);
    }
    ptope_free(mo,o1); ptope_free(mo,o2); ptope_free(mo,o3); ptope_free(mo,o4);
    ap_dimchange_free(a); ap_dimchange_free(r);
  } ENDLOOP;
}
...
void test_permute(void)
{
  ...
  LOOP {
    ...
    o2 = ptope_permute_dimensions(mo,false,o1,p);
    o3 = ptope_permute_dimensions(mo,false,o2,q);
    RESULT(check(o1)); check(o2); check(o3);
    if (ptope_is_neq(mo,o1,o3)) {
      ERROR("not eq");
...
void test_expand(void)
{
  ...
  LOOP {
    ...
    random_poly_ptope(dim, .1, &o1, &p1);
    o2 = ptope_expand(mo,false,o1,d,n); FLAG(mo);
    o3 = ptope_fold(mo,false,o2,dd,n+1); FLAG(mo);
    p2 = ap_abstract0_expand(mp,false,p1,d,n); FLAG(mp);
    p3 = poly_of_ptope(o2);
    RESULT(check(o1)); check(o2); check(o3); check(o3);
    if (flag>=best && ptope_is_neq(mo,o1,o3)) {
      ERROR("not eq");
...
void test_fold(void)
{
  ...
  LOOP {
    ...
    o2 = ptope_fold(mo,false,o1,dd,3); FLAG(mo);
    ...
    if (ap_abstract0_is_eq(mp,p2,p3)) RESULT('*');
    if (flag==exact && !ap_abstract0_is_eq(mp,p2,p3)) ERROR("exact flag");
    ptope_free(mo,o1); ptope_free(mo,o2);
    ap_abstract0_free(mp,p1); ap_abstract0_free(mp,p2); ap_abstract0_free(mp,p3);
  } ENDLOOP;
}
...
void test_widening(void)
{
  ...
  LOOP {
    ...
    ptope_t *o1, *o2, *o;
    o1 = random_ptope(dim,.1);
    o2 = random_ptope(dim,.1);
    o  = ptope_widening(mo,o1,o2);
    RESULT(check(o));
    if (ptope_is_nleq(mo,o1,o) || ptope_is_nleq(mo,o2,o)) {
      ERROR("not upper bound");
      print_ptope("o1",o1); print_ptope("o2",o2); print_ptope("o",o);
    }
    ptope_free(mo,o1); ptope_free(mo,o2); ptope_free(mo,o);
  } ENDLOOP;
  printf("\nwidening convergence\n");
  LOOP {
    int dim = 5, nb = 4*dim*dim;
    ptope_t *o1 = random_ptope(dim,.1);
    for (;nb>0;nb--) {
      ptope_t* o2 = random_ptope(dim,.1);
      ptope_t* o = ptope_widening(mo,o1,o2);
      ptope_free(mo,o2);
      if (ptope_is_leq(mo,o,o1)) { ptope_free(mo,o); break; }
      ptope_free(mo,o1); o1 = o;
    }
    RESULT(check(o1));
    ptope_free(mo,o1);
...
void test_widening_thrs(void)
{
  ...
  LOOP {
    ...
    o = ptope_widening_thresholds(mo,o1,o2,t,10);
    RESULT(check(o));
    if (ptope_is_nleq(mo,o1,o) || ptope_is_nleq(mo,o2,o)) {
      ERROR("not upper bound");
      print_ptope("o1",o1); print_ptope("o2",o2); print_ptope("o",o);
  ...
  LOOP {
    ...
    for (;nb>0;nb--) {
      ptope_t* o2 = random_ptope(dim,.1);
      ptope_t* o = ptope_widening_thresholds(mo,o1,o2,t,10);
      ptope_free(mo,o2);
      if (ptope_is_leq(mo,o,o1)) { ptope_free(mo,o); break; }
      ptope_free(mo,o1); o1 = o;
    }
...
void test_narrowing(void)
{
  ...
  LOOP {
    ...
    ptope_t *o1, *o2, *o, *oo;
    o1 = random_ptope(dim,.01);
    o2 = random_ptope(dim,.01);
    o  = ptope_narrowing(mo,o1,o2);
    oo = ptope_meet(mo,false,o1,o2);
    RESULT(check(o));
    if (ptope_is_nleq(mo,o,o1) || ptope_is_nleq(mo,oo,o)) {
      ERROR("not decreasing");
      print_ptope("o1",o1); print_ptope("o2",o2);
      print_ptope("o",o); print_ptope("oo",oo);
    }
    ptope_free(mo,o1); ptope_free(mo,o2); ptope_free(mo,o); ptope_free(mo,oo);
  } ENDLOOP;
  printf("\nwidening narrowing\n");
  LOOP {
    int dim = 5, nb = 4*dim*dim;
    ptope_t *o1 = random_ptope(dim,.03);
    for (;nb>0;nb--) {
      ptope_t* o2 = random_ptope(dim,.03);
      ptope_t* o = ptope_narrowing(mo,o1,o2);
      ptope_free(mo,o2);
      if (ptope_is_leq(mo,o,o1)) { ptope_free(mo,o); break; }
      ptope_free(mo,o1); o1 = o;
    }
...
void test_assign(int subst, exprmode mode)
{
  ...
  LOOP {
    ...
    random_poly_ptope(dim, .1, &o, &p);
    if (lrand48()%10>=5) ptope_close(pr,o);
    o1 = subst ? ptope_substitute_linexpr_array(mo,false,o,&d,&l,1,NULL) : ptope_assign_linexpr_array(mo,false,o,&d,&l,1,NULL);
    FLAG(mo);
...
void test_par_assign(int subst, exprmode mode)
{
  ...
  LOOP {
    ...
    if (lrand48()%10>=8) ptope_close(pr,o);
    o1 = subst ?
      ptope_substitute_linexpr_array(mo,false,o,d,l,NB_ASSIGN,NULL) :
      ptope_assign_linexpr_array(mo,false,o,d,l,NB_ASSIGN,NULL);
    FLAG(mo);
    p1 = subst ?
...
void test_par_assign2(int subst, exprmode mode)
{
  ...
  LOOP {
    ...
    FLAG(mo);
    o2 = subst ?
      ptope_substitute_linexpr_array(mo,false,o,&d,&l,1,NULL) :
      ptope_assign_linexpr_array(mo,false,o,&d,&l,1,NULL);
    FLAG(mo);
    check(o1); check(o2);
    if (ptope_is_eq(mo,o1,o2)) RESULT('=');
    else if (ptope_is_leq(mo,o1,o2)) RESULT('<');
    else if (ptope_is_leq(mo,o2,o1)) RESULT('>');
    else RESULT('.');
    if (flag==exact && ptope_is_neq(mo,o1,o2)) ERROR("exact flag");
    ptope_free(mo,o); ptope_free(mo,o1); ptope_free(mo,o2);
    ap_linexpr0_free(l);
...
void test_assign_texpr(int subst)
{
  ...
  LOOP {
    ...
    random_poly_ptope(dim, .1, &o, &p);
    if (lrand48()%10>=5) ptope_close(pr,o);
    o1 = subst ? ptope_substitute_texpr_array(mo,false,o,&d,&e,1,NULL) : ptope_assign_texpr_array(mo,false,o,&d,&e,1,NULL);
    FLAG(mo);
...
void test_hash(void)
{
  ...
  LOOP {
    ...
    ptope_t *oa,*ob;
    int ra,rb;
    oa = random_ptope(dim,.1);
    ra = ptope_hash(mo,oa);
    ob = ptope_copy(mo,oa);
    rb = ptope_hash(mo,ob);
    RESULT('.');
    if (ra!=rb) ERROR("different hash");
    ptope_free(mo,oa); ptope_free(mo,ob);
  } ENDLOOP;
}
...
void tests(int algo)
{
  ...
  test_join_array();
  test_add_ray();
  test_add_lincons(expr_ptope);
  test_add_lincons(expr_lin);
  test_add_lincons(expr_interv);
  test_sat_lincons(expr_ptope);
...
int main(int argc, const char** argv)
{
  ...
  pr = ptope_init_from_manager(mo,0,0);
